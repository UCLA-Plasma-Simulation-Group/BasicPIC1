...
module espic1.py in a separate thread, and enters a mainloop.
Then the espic1.py module initializes the physics data, calls the
...
#If running, send a message to continue time loop,
   ...
   def on_plotstart(*args):
      """ callback for PlotChange custom event """
      plot_name = comms.plot_name
      if plot_name=='SMOOTHED ELECTRIC FIELD VS X':
         graphs.dscaler1(comms.plot_data,plot_name,999,0,comms.snx+1,
                         'X')
      elif plot_name=='ELECT VELOCITY DISTR VS VX':
         graphs.displayfv1(comms.plot_data,comms.plot_data2,plot_name,
                            comms.snmv)
      elif plot_name=='ELECTRON VX VERSUS X':
         graphs.grasp1(comms.plot_data,comms.plot_data2,plot_name,999,
                       comms.snx,2,1,comms.snpx,'X')
      elif plot_name=='ENERGY DIFFS VS TIME':
         dtw = comms.sim_data['DTW']
         graphs.displayw1(comms.plot_data,plot_name,0.0,dtw,
                          comms.plot_data2,False)
...
# Launch physics module as a separate thread
   Thread(target=espic1.main).start()
